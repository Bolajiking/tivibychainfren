...
| 2026-07-21 | Mobile dashboard: one consistent chrome, every room reachable | Settings and Videos were bespoke full-screen pages (tiny text back, no bottom nav) — folded into `DashboardShell` so every room wears the same mobile top bar (back to /dashboard, title, optional action) + Channel/Store/Wallet triad. `DashboardShell` gained a `mobileAction` slot. Videos + Settings added to the desktop rail; the overview "Manage" grid covers all rooms. |
| 2026-07-21 | Free follows persist (fans count was stuck at zero) | The "Follow" button was client-only, so `subscriber_count` never moved. New `POST /api/channels/{u}/follow` writes a free-tier subscription row and increments the count once per fan (race-safe RPC, idempotent). The public channel count is computed as the distinct active-subscriber count (never below the column), so creators with existing fans no longer read as zero. |
| 2026-07-21 | Wallet on the creator page | Signed-in users get a header wallet chip (both breakpoints) opening the same global WalletSheet; system tokens, portalled outside the creator theme. |
...
| 2026-07-21 | Mobile creator bar is Dashboard · My channel (was Channel · Store · Wallet) | Store and Wallet duplicated tiles already in the overview's "Manage" grid, spending both spare slots on repeats. The freed slot closes a real gap: on mobile there was no route from the dashboard back to the public page, so a creator checking their own channel had to retype the URL. "My channel" reuses `ViewerTabBar`'s vocabulary for `/{username}`, and the creator page's history-aware `BackButton` closes the round trip. `PRIMARY_DASHBOARD_TABS` narrowed to `overview` in the same change — without that, Store and Money become trapped rooms (the 2026-07-20 Streams/Analytics bug). |
| 2026-07-21 | Fans can save a channel to their phone | The PWA install affordance existed only inside the `isOwner` branch of the channel page, so the one person who never needs it was the only one offered it. `SaveChannelRow` (self-hiding via `useInstallPrompt`, so it never leaves an empty pill in an in-app webview) now sits on the fan action row and in the post-follow capture card — the two capture-ladder moments. Deliberately NOT on the player: stage grammar says video owns the viewport, and an install prompt over a running stream is a toll gate. |
| 2026-07-21 | The channel video shelf is "Videos", not "Replays" | It always rendered uploaded VODs and stream replays from one `videos` array, but the label, the empty state ("No replays yet") and the owner CTA ("Go live") all described only replays — uploading was invisible from the surface it fills. Empty state now offers Upload a video alongside Go live. |
